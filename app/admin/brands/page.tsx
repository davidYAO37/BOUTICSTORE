...
export default function AdminBrandsPage() {
  const [items, setItems] = useState<BrandAdmin[]>([]);
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getBrands({ page, limit: 10, search });
      setItems(data.data || []);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="mb-0">Marques</h2>
        <Link href="/admin/brands/new" className="btn btn-primary d-flex align-items-center gap-2">
          <FaPlus /> Ajouter
        </Link>
      </div>
      ...
      {loading ? <Alert variant="info">Chargement...</Alert> : items.length === 0 ? <Alert variant="warning">Aucune marque</Alert> : (
        <>
          <Table responsive hover className="bg-white shadow-sm rounded">
            <thead><tr><th>Nom</th><th>Slug</th><th>Pays</th><th>Actif</th><th>Actions</th></tr></thead>
            <tbody>
              {items.map((item) => (
                <tr key={item._id}>
                  <td>{item.name}</td>
                  ...
                  <td>
                    <Link href={`/admin/brands/${item._id}`} className="btn btn-sm btn-outline-primary me-2"><FaEdit /></Link>
