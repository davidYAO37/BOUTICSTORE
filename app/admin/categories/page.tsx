...
export default function AdminCategoriesPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getCategories({ page, limit: 10, search });
      setItems(data.data || []);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="mb-0">Catégories</h2>
        <Link href="/admin/categories/new" className="btn btn-primary d-flex align-items-center gap-2">
          <FaPlus /> Ajouter
        </Link>
      </div>
      ...
      {loading ? <Alert variant="info">Chargement...</Alert> : items.length === 0 ? <Alert variant="warning">Aucune catégorie</Alert> : (
        <>
          <Table responsive hover className="bg-white shadow-sm rounded">
            <thead><tr><th>Nom</th><th>Slug</th><th>Parent</th><th>Actif</th><th>Actions</th></tr></thead>
            <tbody>
              {items.map((item) => (
                <tr key={item._id}>
                  <td>{item.name.fr}</td>
                  <td>{item.slug}</td>
                  <td>{item.parent?.name.fr || "-"}</td>
                  <td>{item.isActive ? <Badge bg="success">Oui</Badge> : <Badge bg="danger">Non</Badge>}</td>
                  <td>
                    <Link href={`/admin/categories/${item._id}`} className="btn btn-sm btn-outline-primary me-2"><FaEdit /></Link>
